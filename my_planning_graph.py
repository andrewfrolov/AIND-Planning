...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if action_node.prenodes.issubset(s_level):
                # add all parents for the node
                for literal in s_level:
                    if literal not in action_node.prenodes:
                        continue
    ...
    def add_literal_level(self, level):
        ...
        s_level = set()
        # first, generate s level
        for action_node in a_level:
            s_level |= action_node.effnodes

        # now add parent-child relationship
        for action_node in a_level:
            for s in s_level:
                if s not in action_node.effnodes:
                    continue
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # action 2 negates at least one of the effects of action 1.
        if len(set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)) != 0:
            return True

        # action 1 negates at least one of the effects of action 2.
        if len(set(node_a2.action.effect_add) & set(node_a1.action.effect_rem)) != 0:
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if len(set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos)) != 0:
            return True
        if len(set(node_a1.action.effect_add) & set(node_a2.action.precond_neg)) != 0:
            return True

        if len(set(node_a2.action.effect_rem) & set(node_a1.action.precond_pos)) != 0:
            return True
        if len(set(node_a2.action.effect_add) & set(node_a1.action.precond_neg)) != 0:
            return True
